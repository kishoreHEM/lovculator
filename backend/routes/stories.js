...
router.get("/", async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, story_title, couple_names, love_story, category, mood, 
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!story_title || !love_story) {
      return res.status(400).json({ error: "Story title and content required" });
    }

    const result = await pool.query(
      `INSERT INTO stories (user_id, story_title, couple_names, love_story, category, mood)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
